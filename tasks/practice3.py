...
def get_popular_category(operations: List[Dict[str, Any]]) -> str:
    ...
    for dictionary in operations:
        if dictionary['category'] in d_keys.keys():
            d_keys[dictionary['category']] += dictionary['amount']
        else:
            d_keys[dictionary['category']] = dictionary['amount']
...
def hide_personal_info(info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'passport_code' in info:
        personal_data = info['passport_code']
        for i in range(0, len(personal_data)):
            personal_data = personal_data.replace(str(i), '*')
        info['passport_code'] = personal_data
    if 'phone_number' in info:
        personal_data = info['phone_number']
        for i in range(0, len(personal_data)):
            personal_data = personal_data.replace(str(i), '*')
        info['phone_number'] = personal_data
