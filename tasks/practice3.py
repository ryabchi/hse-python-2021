...
def get_popular_category(operations: List[Dict[str, Any]]) -> str:
    ...
    # впишите ваш код здесь
    global minimal_category
    minimal_amount = -1
    for operation in operations:
         if operation['amount'] > minimal_amount:
             minimal_amount = operation['amount']
             minimal_category = operation['category']
    return minimal_category
...
def hide_personal_info(info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    temp = ""
    if info.get("passport_code", 0):
        for i in info["passport_code"]:
            temp = secure(temp)
        info["passport_code"] = temp

    temp = ""
    if info.get("phone_number", 0):
        for i in info["phone_number"]:
            temp = secure(temp)
        info["phone_number"] = temp
    return info
