...
def generate_payment_message(from_user: str, to_user: str, amount: float) -> str:
    ...
    result = 'Добрый день, {}!\n{}. перевел вам {:.2f} рублей.'.format(to_user.split()[1], from_user.split()[1] + ' ' + from_user.split()[0][0], amount)
    return result
...
def calculate_increased_cashback(operations: List[Tuple]) -> float:
    ...
    # код писать здесь
    for operation in operations:
        result += operation[0] * (0.05 if operation[1] == True else 0.01)
    return result
...
def extract_python_string(raw_string: str) -> str:
    ...
    result = ''
    # код писать здесь
    i = raw_string.lower().index('python')
