...
def generate_payment_message(from_user: str, to_user: str, amount: float) -> str:
    ...
    result = f'Добрый день, {to_user.split()[1]}!\n{from_user_list[1]} {from_user_list[0][0]}. перевел вам {amount:.2f} рублей.'
    return result
...
def clean_user_login(raw_login: str) -> str:
    ...
    """
    login = None
    login = raw_login.strip().replace('"', '').replace('\'', '')
...
def extract_python_string(raw_string: str) -> str:
    ...
    """
    result = ''
    python_start = raw_string.lower().find('python')
    result = raw_string[python_start:python_start + 6]
    return result
