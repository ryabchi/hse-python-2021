...
def generate_payment_message(from_user: str, to_user: str, amount: float) -> str:
    ...
    result = f"Добрый день, {to_user.split()[1]}!\n{from_user.split()[1]} {from_user.split()[0][0]}. перевел вам {amount:.2f} рублей."
    return result
...
def calculate_increased_cashback(operations: List[Tuple]) -> float:
    ...
    # код писать здесь
    for operation in operations:
        result += operation[0]/100*(5 if operation[1] else 1)
    return result
...
def extract_python_string(raw_string: str) -> str:
    ...
    result = ''
    # код писать здесь
    if "python" in raw_string.lower():
        ind = raw_string.lower().index("python")
        result = raw_string[ind:ind+len("python")]
    return result
